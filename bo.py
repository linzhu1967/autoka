...
from use_knowledge import get_concate_logits_with_disagreement
...
def get_avg_combination_representations(output_path):
    teacher_name = ['Z','N','M','G','F','E','D','C','B','A']
    with open(output_path, "w") as writer:
        writer.write("\t".join(["index", "combination_name", "combination_value", "ob_index"])+"\n")
        for i in range (1, 1024):
            ob_i = '{:010b}'.format(i)
            list_i = np.array([int(ii) for ii in ob_i])
            
            teacher_idx = np.nonzero(list_i)[0]
            weighted_i = list_i/len(teacher_idx)
            weighted_i_str = "-".join([str(w) for w in weighted_i])
            # print(teacher_idx)
            # print(type(teacher_idx))

            combination_name = "".join(sorted([teacher_name[idx] for idx in teacher_idx]))
            print(i, combination_name, weighted_i_str, ob_i)
            writer.write("\t".join([str(i), combination_name, weighted_i_str, ob_i])+"\n")
...
# todo
def sample(combination_name, device, random_value):

    if len(combination_name) == 1:
        pqau_saved_path="/home/ylz/ad-hoc_datasets/PubMedQA/knowledge2/pqau/teacher_{}.jsonl".format(combination_name)
        pqal_dev_saved_path="/home/ylz/ad-hoc_datasets/PubMedQA/knowledge2/pqal_dev/teacher_{}.jsonl".format(combination_name)
    else:
        pqau_saved_path="/home/ylz/ad-hoc_datasets/PubMedQA/knowledge2_kldiv4.0/pqau/teacher_{}.jsonl".format(combination_name)
        pqal_dev_saved_path="/home/ylz/ad-hoc_datasets/PubMedQA/knowledge2_kldiv4.0/pqal_dev/teacher_{}.jsonl".format(combination_name)
    
    local_rank = int(os.getenv('LOCAL_RANK', -1))
    if local_rank in [-1,0]:
        # print("local_rank({}) start get_dis_avg_knowledge_by_combination_name".format(local_rank))
        dis_threshold = 4.0
        get_concate_logits_with_disagreement(combination_name, pqau_saved_path, pqal_dev_saved_path, dis_threshold)
    dist.barrier()

    if len(combination_name) == 1:
        # (avg logits)+(psuedo label)
        acc, f1_macro = new_simple_ka_process(combination_name, [pqau_saved_path, pqal_dev_saved_path],device, random_value)
    else:
        acc, f1_macro = new_ka_process(combination_name, [pqau_saved_path, pqal_dev_saved_path],device, random_value)
    return [acc, f1_macro]
...
def cal_EI(mu, cov, best_sample):
    ...
    # xi = 0.01 
    xi = 0.001 # ablation study exp-5, xi=0.001
...
if __name__ == "__main__":
    ...
    rep_path = "/home/ad-hoc_datasets/PubMedQA/knowledge2_kldiv4.0/avg_combination_representations.tsv"
    acc_path = "/home/ad-hoc_datasets/PubMedQA/knowledge2_kldiv4.0/random2431_avg_combination_accuracy.tsv"
    ...
    sampled_combinations = np.array([])

    # init with 10 teacher models and 10 teacher combinations
    init_single_teachers = [1, 2, 4, 8, 16, 32, 64, 128, 256, 512]
    ...
    for i, c_idx in enumerate(init_sampled_combinations):
        if c_idx not in sampled_combination_accuracy_dict.keys() or len(init_sampled_combinations) <10:
            ...
            acc, f1_macro = sample(c_name, device, seed_value)
    ...
    sampled_combinations = np.append(sampled_combinations, init_sampled_combinations)
    print("length of sampled combinations: {}".format(len(sampled_combinations)))
    # sampled_labels = np.isin(all_combinations_idx, sampled_combinations)
    ...
    for i in range(num_sample):
        ...
        if next_combination_index in sampled_combination_accuracy_dict:
            ...
            acc, f1_macro = sample(next_combination_name, device, seed_value)
